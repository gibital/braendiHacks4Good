...
# # Allowed multipliers (if user leaves blank, a random one is chosen).
...
#     else:
#         multiplier = random.choice(allowed_multipliers)
#     employee_target_hours[e] = 420 * multiplier  # target hours for the 70-day period
...
# # Use day names (e.g., Monday, Tuesday). We'll map these to indices: Monday=0, Tuesday=1, ..., Sunday=6.
# day_name_to_index = {
#     'monday': 0,
#     'tuesday': 1,
#     'wednesday': 2,
...
# #   - Otherwise, if the user did not specify individual unavailability, then for weekdays use simulation (70% available) and weekends available.
# availability = {}
# for e in employees:
#     availability[e] = {}
#     for d in days:
#         dow = d % 7
#         # First, enforce never available days-of-week.
#         if dow in never_available[e]:
#             availability[e][d] = False
#         # Next, if the user specified individual unavailability, use that.
#         elif d in individual_unavailable[e]:
#             availability[e][d] = False
#         else:
#             # Otherwise, if no individual unavailability was provided, simulate for weekdays.
...
#     with pd.ExcelWriter('outputs/weekly_schedule.xlsx') as writer:
